...
function getTn($invId) {
 $db = acmeConnect();
 $sql = "SELECT imgId, imgPath, imgName, imgDate, inv.invId, invName, invImage from images im INNER JOIN inventory inv ON im.invId = inv.invId WHERE im.invId = :invId and im.imgName LIKE '%-tn%'";
 $stmt = $db->prepare($sql);
 $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
 $stmt->execute();
